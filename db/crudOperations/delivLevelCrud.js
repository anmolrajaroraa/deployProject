...
// setVendorDelDateStatus(delivery){
// return function(cb){
//     DelivDateStatusSchema.DelStatus.findOneAndUpdate({deliveryId:delivery.deliveryId},{$push:{}})
// }

//} 

getLastDelLevel(delId){
    return function(cb){
        DelivDateStatusSchema.DelStatus.findOne({deliveryId:delId,"deliveryDates.date":null},
        {"deliveryDates":{$elemMatch:{date:null}}},(err,doc)=>{
            if(err){
        return cb('DB Error');
            }else if(doc==null){
         return cb('No Level Found');
            }else{
                console.log(doc);
                cb(null,doc)
            }
...
setDynamicDelForwardStatus(){
    return function(obj,cb){
   console.log('Herer ------',obj)
       DelivDateStatusSchema.DelStatus.findOne({deliveryId:obj.deliveryId},(err,doc)=>{
           if(err){
               return cb('DB Error');
           }else if(doc==null){
               return cb('No Doc Found');
           }else{
               DelivDateStatusSchema.DelStatus.findOneAndUpdate({deliveryId:obj.deliveryId,
                "deliveryDates.levelValue":obj.level},
               {$set:{"deliveryDates.$.date":obj.date}},(err,doc1)=>{
                   if(err){
             return cb('DB Error');
                   }else if(doc1==null){
    return cb('Error');
...
setDelDateStatus(dellevelobj,res){
    DelivLevSchema.Deliverylevel.findOne({maxLevel:dellevelobj.levelValue},(err,doc)=>{
        if(err){
           ...
           DelivDateStatusSchema.DelStatus.findOneAndUpdate({deliveryId:dellevelobj.deliveryId},
            {$set:{deliveryDates:levelArray}},{new:true},(err,doc)=>{
            if(err){
                res.status(409).json({status:config.ERROR,message:'DB Error'})
            }else if(doc==null){
...
OrderSchema.StandardDelivery.findOneAndUpdate({deliveryId:levelobj.deliveryId},
    {$set:{maxLevel:levelobj.levelValue}},{new:true},(err,doc)=>{
    if(err){
        res.status(409).json({status:config.ERROR,message:'DB Error'})
    }else if(doc==null){
    ...
    delivLevelCrud.setDelDateStatus(levelobj,res);
        ...
        OrderSchema.ExpressDelivery.findOneAndUpdate({deliveryId:levelobj.deliveryId},
            {$set:{maxLevel:levelobj.levelValue}},{new:true},(err,doc)=>{
            if(err){
                res.status(409).json({status:config.ERROR,message:'DB Error'})
            }else if(doc==null){
...
editdelivLevel(delivlevel,res){
    console.log(delivlevel);
    DelivLevSchema.Deliverylevel.findOneAndUpdate({maxLevel:delivlevel.maxLevel},
        {$set:{maxLevel:delivlevel.maxLevel, levels:delivlevel.levels}},(err,doc)=>{
        if(err){
            res.status(409).json({status:config.ERROR,message:'DB Error'})
        }else if(doc==null){
