...
res.status(409).json({status:config.EMPTY,message:'No Data Found'});
    ...
    async uploadProducts(req,res,categories,subcategories,products,subProducts){
        ...
        //    })
             ...
             Products.SubProduct.ensureIndexes((err)=>{
                 if(err){
                     console.log(err);
                     return cb('DB Error');
                 }else{
    ...
    editProducts(req,res){
      //  console.log(req.body.priceAndAmount);
        Products.SubProduct.findOneAndUpdate({subproductId:req.body.stackTrace[3]},{$set:{
            "info.foodPreference":req.body.foodPreference,
                ...
                let priceArray=[];
                     ...
                     //   console.log(obj.info.isExpress);
                        let obj1={
                            ...
                            info:{
                                foodPreference:obj.info.foodPreference,
...
filteredSearch(res,searchobj){
        var query={};
        query['$and']=[];

  if(searchobj.categoryName!=null){
    query['$and'].push({categoryName:searchobj.categoryName})
  }

  if(searchobj.subcategoryName!=null){
    query['$and'].push({subcategoryName:searchobj.subcategoryName})
  }
  
  if(searchobj.productName!=null){
    query['$and'].push({productName:searchobj.productName})
  }
console.log(searchobj);
        let params= searchobj.params;
                ...
                if(params[key].length>0){
                    ...
                    for(let discount of params[key]){
                        ...
                        orQuery['$or'].push({"info.priceAndAmount":{$elemMatch:{discount:
                            {$gt:parseInt(discount.split(' ')[0]),$lte:parseInt(discount.split(' ')[1])}}}});
                    }
                    query['$and'].push(orQuery);
                    console.log(orQuery.$or[0]['info.priceAndAmount']) 
                }
        ...
        Products.SubProduct.find(query,(err,docs)=>{
            ...
            if(err){
                ...
                res.status(403).json({status:config.ERROR,message:'No Product Found',docs:[]});
            }
            else{
