 ...
 const AddressotpCrud=require('../../Utils/email/otpEmail/generateotp');
const customerCrud={
...
checkOrderAddressOTP(otp,secret,res){
 // console.log(AddressotpCrud.checkOtp(otp))
if(AddressotpCrud.checkOtp(otp,secret)!=false){
...
mailUser(emailobj.email,emailobj.firstName).then(data=>{
    if(data!=null){
      //  console.log(data);
        res.status(200).json({status:config.SUCCESS,isCreated:true,timeRemain:data.timeRemain,secret:data.secret});
    }
